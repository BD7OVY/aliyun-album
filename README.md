...
面向培训场景：摄影师通过**腾讯微云收集链接**上传照片 → 你回家双击 `publish.bat` → 照片自动发布到 GitHub Pages 在线相册 → 分享链接给所有人查看。

**不需要带电脑去现场**，只需要把微云收集链接分享给摄影师即可。

## 工作流

```
摄影师打开微云收集链接 → 免登录上传照片（手机/电脑都行）
                                    │
                    你回家后从微云下载照片到 incoming/ 文件夹
                                    │
                            双击 publish.bat
...
> 说明：阿里云盘个人版没有「允许上传」功能（那是企业版才有），所以收集环节用腾讯微云——免费、微信/QQ 直接登录、对方免登录上传。阿里云盘仍作照片备份。

1. 浏览器打开 **https://www.weiyun.com/** → 用 **QQ 或微信扫码登录**
2. 新建一个文件夹（如「培训照片」）
3. **右键该文件夹 → 点「收集文件」**
4. 复制生成的链接（可设置截止时间），把链接发给摄影师 —— 他们**免登录**、手机电脑都能传，照片自动进你的微云文件夹
5. 收集完毕可在文件夹上右键「关闭收集」

### 第 3 步：下载照片 + 发布

1. 从微云把收集到的照片下载到 `incoming/` 文件夹
2. 双击 **`publish.bat`**
3. 脚本自动完成：
   - 内容哈希去重（同名副本自动跳过）
...
```
培训现场：摄影师收到微云收集链接 → 拍完就传（手机直接传）
        ↓
你回家：微云下载照片到 incoming/ → 双击 publish.bat → 完成
        ↓
